---
title: "Atelier 2 - Outils de base et programmation"
output: 
  html_document: 
    highlight: haddock
    number_sections: yes
...
.module(numModule = 2, "Outils de base et programmation")
```

L'objectif de cet atelier est de vous amener à **manipuler les briques élémentaires du langage de R : vecteurs, matrices et listes**. À ce titre, il s'agit d'un détour indispensable avant d'aborder les opérations plus complexes portant sur les tables de données (sélection d'observations et de variables, tri, fusion, etc.). 
...
La **fonction `c()`**`r .fun("c", bold = TRUE)` permet de créer des vecteurs (de nombre, de charactères ou de booléens) : 
```{r}
...
- utiliser un **vecteur logique de même longueur** (comme dans des sélections sur conditions).
...
L'utilisation de vecteurs logique pour extraire des valeurs est particulièrement importante, dans la mesure où elle intervient dans la plupart des opérations de **sélection d'observations ou de variables** dans une table de données.
...
- la fonction `seq()``r .fun("seq", bold = TRUE)` ** produit des séquences de nombres**. Dans les cas courants, elle peut être remplacée par `:``r .fun(":", bold = TRUE)` :
    ```{r}
...
```{r, results = "asis", echo = FALSE}
.souspartie("Matrices : création et sélection")
```
...
**Le type d'objet utilisé pour stocker des données statistiques, le `data.frame` présente des points communs avec les matrices (accès aux objets par deux positions, utilisation de fonctions adaptées aux objets en deux dimensions, etc.).** 
...
```{r, results = "asis", echo = FALSE}
.souspartie("Matrices : opérations spécifiques")
```
...
```{r, results = "asis", echo = FALSE}
.partie("Manipulation de listes")
```

```{r, results = "asis", echo = FALSE}
.souspartie("Les listes : création et sélection")
```
...
```{r, results = "asis", echo = FALSE}
.souspartie("Les listes : opérations")
```
...
```{r, results = "asis", echo = FALSE}
.partie("Éléments de programmation")
```

R est un langage de programmation aux caractéristiques usuelles permettant d'effectuer des tests conditionnels selon le structure `if` ... `then`. Les boucles permettent de répéter des instructions. Enfin, des fonctions de type `apply` existant par défaut sont optimisées pour s'appliquer à tous les éléments des matrices ou listes. 

```{r, results = "asis", echo = FALSE}
.souspartie("Les conditions")
```

Pour des conditions à valeurs booléenne non vectorielles, la syntaxe est la suivante : 
```{r}
...
Il est également possible d'appliquer des conditions sur des vecteurs grâce à la fonction `ifelse` :

```{r}
x <- -3:3
# si on prend la racine carrée sans réfléchir, R retourne un avertissement avec des valeurs manquantes
sqrt(x) 
# la même chose sans avertissement
sqrt(ifelse(x >= 0, x, NA)) 
# si on souhaite retourner une valeur nulle (par exemple) 
sqrt(ifelse(x >= 0, x, 0)) 
```

```{r, results = "asis", echo = FALSE}
.souspartie("Les boucles")
```

Il est possible d'utiliser les boucles classique de la programmation `for`, `while` et `repeat`. Elles ne sont pas efficaces sous R car lentes. Il est préférable de raisonner en vectoriel (cf. `apply` ci-après). 
...
Les boucles `for` peuvent être numériques entières comme l'exemple précédent mais aussi s'appuyer sur une liste, un vecteur de caractères, des valeurs de variables, etc.

L'instruction `break` permet de quitter une boucle tandis que `stop("message")` arrête la boucle avec le message d'erreur `message` qui est affiché.
...
```{r, results = "asis", echo = FALSE}
.souspartie("Les fonctions de type `apply`")
```

Le langage R est vectoriel et les boucles sont chronophages. Il est recommandé de les éviter et d'utiliser les fonctions matricielles de type `apply`. Elles permettent d'appliquer sur des vecteurs, des matrices, des listes d'appliquer une même fonction de façon analogue à `lapply` vu précédemment. 

Il a plusieurs manières de faire la somme par ligne d'une matrice. La première est de faire une boucle :

```{r}
# on déclare la matrice
M <- matrix(1:20, nrow=5, ncol=4)
# vecteur de résultat (vide pour l'instant)
res1 <- vector("numeric", 5)

for (i in 1:nrow(M)) # boucle sur les lignes
{
  case <- 0 
  for (j in 1:ncol(M)) # pour chaque ligne, somme sur les colonnes
  {
    case <- case + M[i,j]
  }
  res1[i] <- case
...
Une solution plus brève et plus rapide est d'appliquerla fonction `sum` en ligne :
```{r}
# On applique la fonction sum() par ligne : MARGIN = 1
res2 <- apply(M,MARGIN = 1,FUN=sum)
# Le résultat est bien le même (et c'est plus rapide)
identical(res1, res2)

# Pour faire la somme par colonne, il faut indiquer la marge 2 :
apply(M,2,sum)

```

Les fonctions `sapply` et `lapply` s'appliquent respectivement aux vecteurs et aux listes. La fonction `tapply` permet en outre d'ajouter une condition de type `by` en regroupant selon les valeurs d'une variable de la table. Leur utilisation plus fine sera détaillée dans la formation de perfectionnement à venir. 
