---
title: "Atelier 3 - Statistiques de base sur données"
output: 
  html_document: 
    highlight: haddock
    number_sections: yes
...
.module(numModule = 3, "Atelier 3 - Statistiques de base sur données")
```


L'objectif de ce troisième atelier est de **réutiliser dans un cadre \og métier \fg{} les briques élémentaires du langage** introduites dans le module précédent: 
...
**Remarque**  Par défaut, **la fonction `data.frame()` convertit les variables caractères en facteurs**. Pour éviter ce comportement (pas toujours souhaitable), il suffit d'utiliser l'**argument `stringsAsFactors = FALSE`**.
```{r}
...
a. Après avoir modifié le répertoire de travail avec `setwd()`, utilisez la fonction `readRDS()``r .fun("readRDS")` pour charger le fichier `eect4.rds` dans l'objet `eec` (fonction associée à `saveRDS()``r .fun("saveRDS")`permettant de lire des données enregistrées avec R).
...
c. On souhaite créer deux nouvelles tables ne contenant que les variables sur lesquelles portent différents aspects de l'étude :
    ...
    ii. `eec3` qui contienne toutes les variables de `eec` à l'exception  de `cse`.`r .fun("setdiff")`
        ...
        ```{r, results = "asis", echo = FALSE}
        .indic("**Indication** Comment utiliseriez-vous les opérations ensemblistes sur les vecteurs pour créer le vecteur des noms des variables de la table `eec` SAUF `cse` ? Utilisez-le comme au i. pour sélectionner toutes les variables sauf `cse`.")
        ```
...
À partir de la table `eec`, on souhaite produire une nouvelle table (`eec5`) qui ne comporte qu'un individu par ménage, le plus âgé. Les ménages sont identifiés par la variable `ident` et la variable `age` code l'âge des individus. 

a. Comment détermineriez-vous le nombre de ménages dans la table `eec` ?`r .fun("unique")`
    ```{r, results = "asis", echo = FALSE}
...
b. On cherche d'abord à constituer une table ne comportant qu'un seul individu par ménage, quel que soit son âge. Comment procéderiez-vous ?`r .fun("duplicated")`
...
c. Comment adapteriez-vous la réponse à la question précédente pour sélectionner l'individu le plus âgé du ménage (sans chercher à maîtriser celui qui est sélectionné quand plusieurs membres d'un même ménage ont le même âge) ?`r .fun("order")`
...
d. (Optionnel) Comment adapteriez-vous la réponse à la question précédente pour effectuer un tirage au sort quand plusieurs membres d'un même ménage ont le même âge ?`r .fun("rnorm")`
...
# Comparaison des trois méthodes + variante optimisée de sapply()
...
a. On souhaite calculer le taux de chômage au niveau national et régional. Le taux de chômage est défini par le ratio du nombre total d'individus au chômage (`acteu %in% "2"`) sur la taille de la population active (`acteu %in% c("1", "2")`).
    ...
    **Remarque** Le fichier utilisé ici ne comporte que les logements en première ou sixième interrogation : les estimations effectuées dans ce cas pratique n'ont donc aucune raison de coïncider avec les estimations officielles (qui intègrent également des corrections sur les variations saisonnières et les jours ouvrables). 
...
Au-delà de ces fonctions natives, **le *package* `descr` facilite considérablement l'analyse uni- et bi-variée de variables qualitatives**, en particulier quand les données ont à être pondérées (données d'enquête).
...
- `barplot()``r .fun("barplot", bold = TRUE)` produit le diagramme en bâtons représentant la fréquence d'une variable qualitative. 
...
```{r, results = "asis", echo = FALSE}
.partie("Une brève introduction à `ggplot2`")
```

`ggplot2` est un package développé par Hardley Wickham et fondé sur la "grammaire des graphiques" proposée par Leland Wilkinson. Il permet de réaliser un vaste ensemble de graphiques et de nombreux autres packages ont vu le jour pour étendre ses capacités en en reprenant le principe.

Voici quelques exemples (réalisés par d'autres) :
...
Il existe deux fonctions qui permettent de réaliser des graphiques dans ce package : `qplot()``r .fun("qplot", bold = TRUE)` et `ggplot()``r .fun("ggplot", bold = TRUE)`. `qplot()``r .fun("qplot", bold = TRUE)` permet de produire simplement des graphiques et possède une syntaxe plus simple. 

`qplot()``r .fun("qplot", bold = TRUE)` s'utilise de façon relativement semblable à `plot()``r .fun("plot", bold = TRUE)` :
```{r warning=FALSE}
qplot(Sepal.Length, Sepal.Width, data=iris)
qplot(Sepal.Length, Sepal.Width, colour=Species, shape=Species, data=iris) 
#on règle la forme et la couleur en fonction de l'espèce
```

L'utilisation est simple et donne des graphiques attractifs. L'option `geom` permet de préciser le type de graphique :
```{r message=FALSE}
qplot(Species,Sepal.Length, data=iris, geom="boxplot") #boîte à moustache
qplot(Sepal.Length, data=iris, geom="histogram") #histogramme
qplot(Sepal.Length, data=iris, geom="density") #densité
```


Cette fonction fonctionne avec des variables tandis que `ggplot()``r .fun("ggplot", bold = TRUE)` fonctionne avec des bases au format `data.frame`. `ggplot()``r .fun("ggplot", bold = TRUE)` possède une syntaxe ainsi plus rigoureuse et davantage de fonctionnalités.

Le principe sous-jacent est une grammaire des graphiques qui permet de construire une grande variété à partir des données, des coordonnées des points et des calques `geom`. Les propriétés esthétiques des graphiques sont des options.

L'exemple classique est d'utiliser la table `diamonds` qui existe sur R. Il faut la charger préalablement.

```{r}
data("diamonds") #la table est chargée dans l'environnement
names(diamonds) #nom des variables
?diamonds #description

# On trace le prix en fonction du poids
ggplot(diamonds, aes(x=carat, y=price)) + geom_point() 
# On rend les points transparents à 1/20ème
ggplot(diamonds, aes(x=carat, y=price)) + geom_point(alpha=0.05) 
# On affiche les couleurs différemment, selon la variable color
g <- ggplot(diamonds, aes(x=carat, y=price, colour=color)) + geom_point(alpha=0.05) 
# On peut changer de palettes de couleur 
g + scale_colour_brewer(palette = 1)

```

Il est également possible de faire des diagrammens en barre via la commande `geom_bar()``r .fun("geom_bar", bold = TRUE)`.

```{r}
# On trace la clarté en fonction de la taille
ggplot(diamonds, aes(clarity, fill = cut)) + geom_bar()
# D'autres options de positionnement sont possibles 
ggplot(diamonds, aes(clarity, fill = cut)) + geom_bar(position="fill")

ggplot(diamonds, aes(clarity, fill = cut)) + geom_bar(position="dodge")
```

Tous les éléments de la légende (titre, labels, etc.) sont modifiables, via des ajouts d'options : 

```{r}
g + ggtitle("Prix en fonction du carat")
g + ylab("Prix") #axe des ordonnées, xlab() pour les abscisses
g + theme(legend.position = "bottom") 
```

La fonction `ggsave()``r .fun("ggsave", bold = TRUE)` permet de sauvegarder le dernier graphique affiché, en indiquant le type de fichier directement dans l'extension (`.pdf`, `.png`, `.jpeg`, etc.).

Le *package* `ggplot2` permet de faire une grande variété de graphiques d'une, deux ou trois variables. Des galeries d'exemples utilisant `ggplot2` se trouvent en ligne, comme [celle-ci](https://www.r-graph-gallery.com/portfolio/ggplot2-package/) ou [celle-là](https://plot.ly/ggplot2/geom_ribbon/).
...
Vous pouvez retrouver des supports très proches de cette formation (qui en est directement inspirée) sur le site de Martin Chevalier (Insee) : [r.slmc.fr](https://teaching.slmc.fr/r/).

```{r,results = "asis",echo = FALSE}
.souspartie("Formation R perfectionnement")
```

Le support de la formation R perfectionnement est également en ligne à l'adresse : [t.slmc.fr/perf](http://teaching.slmc.fr/perf). Elle aborde trois sujets : 
...
R est un langage riche, il permet de produire des applications via  [R Shiny](https://shiny.rstudio.com/) ou le support de cette formation via [R Markdown](https://rmarkdown.rstudio.com/).
...
Ce  [site](http://factominer.free.fr/) est une porte d'entrée vers l'utilisation du package `FactoMineR`. 
...
```{r,results = "asis",echo = FALSE}
.souspartie("Les `cheatsheets` à ne pas manquer")
```

Les *cheatsheets* sont des aides-mémoire synthétiques rassemblant les fonctions courantes pour un package ou des fonctionnalités communes. Généralement tenant sur un recto-verso, elles sont très pratiques d'utilisation une fois imprimées. Il en existe de nombreuses, souvent en anglais, les plus courantes sont téléchargeables par sur la page de [R studio](https://www.rstudio.com/resources/cheatsheets/). Il y a aussi des *Reference cards* regroupant des listes de fonctions avec une brève description de leur usage, par exemple ces [commandes usuelles](http://julien.cremeriefamily.info/doc/teachings/commandes_r.pdf) (en français).

Vous y trouverez notamment de l'aide précieuse pour: 

- [l'interface R Studio](https://www.rstudio.com/wp-content/uploads/2016/02/IDE-french.pdf), en français

- [le langage R de base](http://github.com/rstudio/cheatsheets/raw/master/base-r.pdf)

- [R avancé](https://www.rstudio.com/wp-content/uploads/2016/02/advancedR.pdf)

- [les graphiques avec gglot2](https://www.rstudio.com/wp-content/uploads/2015/06/ggplot2-french.pdf), en français

- [les packages dplyr et tidyr](https://www.rstudio.com/wp-content/uploads/2016/01/data-wrangling-french.pdf), en français


- [R Markdown](https://github.com/rstudio/cheatsheets/raw/master/rmarkdown-2.0.pdf)

- [R Shiny](https://github.com/rstudio/cheatsheets/raw/master/shiny.pdf)




```{r, results = "asis", echo = FALSE}
.partie("Application à l'enquête Pisa")
```
...
a. Utilisez la fonction `read_sas()``r .fun("read\\_sas")` du *package* `haven` pour importer les deux fichiers dans les objets `stu` et `sch` respectivement. Afin de faciliter les exploitations futures, passez leurs noms de variables en minuscules`r .fun("names")`. 
